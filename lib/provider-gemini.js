...
// Model:      a native-audio Live model (Gemini 3.1 Flash Live Preview) - this
//             produces the spoken audio (TTS) natively and transcribes the mic.
// Mic in:     16 kHz PCM16 chunks as realtimeInput.audio
...
export class GeminiLiveProvider {
  ...
  async connect() {
    ...
    processor.onaudioprocess = (e) => {
      ...
      this.send({
        realtimeInput: {
          audio: { mimeType: 'audio/pcm;rate=16000', data: b64FromBytes(new Uint8Array(pcm.buffer)) }
        }
      });
    };
    ...
    await new Promise((resolve, reject) => {
      ...
      ws.onclose = (event) => {
        ...
        const detail = this.#closeDetail(event);
        if (!this.setupDone) reject(new Error(`Gemini closed the connection before setup${detail} - check the API key and model name on the Settings page.`));
        else this.cb.onClosed?.(`Connection lost${detail}`);
      };
    });
  }
  ...
  async #onMessage(event) {
    ...
    if (msg.setupComplete) {
      ...
      this.#sendText('Greet me in one short sentence and say you are ready.');
      return;
    }
  ...
  requestResponse() {
    this.#sendText('Respond now based on the latest context.');
  }
  ...
  #closeDetail(event) {
    const pieces = [];
    if (event?.code) pieces.push(`code ${event.code}`);
